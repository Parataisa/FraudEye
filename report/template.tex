...
We selected the Transaction dataset which includes ~227000 Data points each having 30 feature(1 Time + 29 other features). The Data were labeled with a 1 if the transaction was fraudulent and 0 if it was not.
The dataset is highly imbalanced with only ~0.001729\% of the data points being labeled as fraudulent. The nature of the task is a classification problem either a transaction is fraudulent or not.
The 29 other features are anonymized, and we do not know what they represent. The dataset has no missing values/features.
{\color{blue}
\begin{itemize}
...
Due to the imbalanced nature of the dataset, we used an oversampling technique to balance the dataset. Meaning we duplicated the fraudulent transactions to a certain percentage of the training data. Around 40\%-50\% showed the best results.
